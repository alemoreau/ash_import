defmodule AshImport.Transformation.Sum do
  @moduledoc """
  Sums numeric values.
  """
  ...
  def transform(values, _opts, _context) when is_list(values) do
    case convert_to_numbers(values) do
      {:ok, numbers} -> {:ok, Enum.sum(numbers)}
      {:error, _} = error -> error
    end
  end
  ...
  def transform(_values, _opts, _context) do
    {:error, "Sum transformation expects a list of values"}
  end

  @impl true
  def describe(_opts), do: "Sums numeric values"
